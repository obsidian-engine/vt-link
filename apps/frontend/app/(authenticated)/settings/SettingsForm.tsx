...
import type { UserSettings, UpdateUserSettingsRequest } from "@/lib/api-client"
...
export function SettingsForm({ initialSettings }: SettingsFormProps) {
  ...
  useEffect(() => {
    if (initialSettings) {
      setTimezone(initialSettings.timezone || "asia-tokyo")
      setLanguage(initialSettings.language || "ja")
      setDefaultReplyDelay(initialSettings.defaultReplyDelay || 0)
      setNotificationEnabled(initialSettings.notificationEnabled ?? true)
    }
  }, [initialSettings])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.PUT('/api/v1/settings', { body: updateData })

      if (response.error) {
        throw new Error(response.error.message)
      }

      // 成功後、Server Componentを再取得するためにルーターをリフレッシュ
      router.refresh()
      setSaveSuccess(true)
