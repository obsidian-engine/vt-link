...
export function useMessages(page = 1, limit = 20) {
  return useSWR(
    ['/api/v1/messages', 'listMessages', page, limit],
    async () => {
...
export function useMessage(id: string) {
  return useSWR(
    id ? ['/api/v1/messages/{id}', 'getMessage', id] : null,
    async () => {
...
export async function createMessage(input: CreateMessageRequest) {
  const res = await client.POST('/api/v1/messages', { body: input })
  if (res.error) throw res.error
  return res.data
}

export async function updateMessage(id: string, input: UpdateMessageRequest) {
