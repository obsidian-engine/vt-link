...
import type { Campaign, CreateCampaignRequest, UpdateCampaignRequest } from '@vt-link/schema-zod'

const client = makeClient()

export function useCampaigns(page = 1, limit = 20) {
  return useSWR(
    ['/api/v1/campaigns', 'listCampaigns', page, limit],
    async () => {
...
export function useCampaign(id: string) {
  return useSWR(
    id ? ['/api/v1/campaigns/{id}', 'getCampaign', id] : null,
    async () => {
