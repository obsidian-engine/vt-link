...
export class ListHistoryUsecase {
  ...
  async execute(input: ListHistoryInput): Promise<ListHistoryOutput> {
    ...
    if (input.status) {
      ...
      campaigns = await this.#campaignRepository.findByAccountId(
        input.accountId,
      );
    }
    ...
    const campaignHistoryItems: CampaignHistoryItem[] = paginatedCampaigns.map(
      (campaign) => ({
        id: campaign.id,
        name: campaign.name,
    ...
    const statistics = await this.#campaignRepository.getAccountStatistics(
      input.accountId,
    );
