...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[BATCH_EXECUTOR] Starting batch execution at ${currentTime.toISOString()}`,
    );
    ...
    if (!channelAccessToken) {
      console.error(
        "[BATCH_EXECUTOR] LINE_CHANNEL_ACCESS_TOKEN is not configured",
      );
      return NextResponse.json(
    ...
    console.log(
      `[BATCH_EXECUTOR] Found ${readyBatches.length} batches ready for execution`,
    );
    ...
    const results = await Promise.allSettled(
      readyBatches.map(async (batch, index) => {
        try {
          ...
          console.error(
            `[BATCH_EXECUTOR] Failed to execute batch ${batch.id}:`,
            errorMessage,
          );
