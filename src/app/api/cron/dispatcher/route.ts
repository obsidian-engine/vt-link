...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[CRON] Starting scheduled delivery check at ${currentTime.toISOString()}`);
    ...
    // 送信準備ができたキャンペーンを取得
    const readyCampaigns = await campaignRepository.findScheduledReadyToSend(currentTime);
    ...
    console.log(`[CRON] Found ${readyCampaigns.length} campaigns ready for delivery`);
    ...
    const results = await Promise.allSettled(
      readyCampaigns.map(async (campaign) => {
        try {
          ...
          console.log(`[CRON] Campaign ${campaign.id} processed successfully. Batches created: ${result.batchesCreated}, Total recipients: ${result.totalRecipients}`);
    ...
    const totalBatchesCreated = results
      ...
      .reduce((sum, r) => sum + (r.status === 'fulfilled' ? r.value.batchesCreated || 0 : 0), 0);
      
    const totalRecipients = results
      .filter(r => r.status === 'fulfilled' && r.value.status === 'success')
      .reduce((sum, r) => sum + (r.status === 'fulfilled' ? r.value.totalRecipients || 0 : 0), 0);
