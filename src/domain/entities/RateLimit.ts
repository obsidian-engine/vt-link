...
export class RateLimit {
  ...
  private constructor(scope: RateLimitScope, limit: number, windowSeconds: number) {
    this.#scope = scope;
    this.#limit = limit;
  ...
  static create(scope: RateLimitScope, limit: number, windowSeconds: number): RateLimit {
    if (limit <= 0) {
  ...
  static reconstruct(scope: RateLimitScope, limit: number, windowSeconds: number): RateLimit {
    return new RateLimit(scope, limit, windowSeconds);
  }
  ...
  generateKey(userId: string, groupId?: string, roomId?: string): string {
    switch (this.#scope) {
      ...
      case RateLimitScope.Group:
        return groupId ? `rate_limit:group:${groupId}` : `rate_limit:user:${userId}`;
      case RateLimitScope.Room:
        return roomId ? `rate_limit:room:${roomId}` : `rate_limit:user:${userId}`;
      default:
