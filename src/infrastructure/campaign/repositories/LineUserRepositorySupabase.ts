...
export class LineUserRepositorySupabase implements LineUserRepository {
  ...
  async findByCriteria(
    ...
    if (error) {
      throw new Error(
        `Failed to find LINE users by criteria: ${error.message}`,
      );
    }
  ...
  async countByCriteria(
    ...
    if (error) {
      throw new Error(
        `Failed to count LINE users by criteria: ${error.message}`,
      );
    }
  ...
  async getStatistics(accountId: string): Promise<{
    ...
    for (const result of results) {
      if (result.error) {
        throw new Error(
          `Failed to get user statistics: ${result.error.message}`,
        );
      }
    }
