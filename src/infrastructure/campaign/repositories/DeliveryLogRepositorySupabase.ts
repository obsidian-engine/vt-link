import {
  DeliveryLog,
  DeliveryStatus,
} from "@/domain/campaign/entities/DeliveryLog";
...
export class DeliveryLogRepositorySupabase implements DeliveryLogRepository {
  ...
  async findByBatchId(batchId: string): Promise<DeliveryLog[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by batch: ${error.message}`,
      );
    }
  ...
  async findByCampaignId(
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by campaign: ${error.message}`,
      );
    }
  ...
  async findByStatus(
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by status: ${error.message}`,
      );
    }
  ...
  async findByDateRange(
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by date range: ${error.message}`,
      );
    }
  ...
  async findByCampaignIdAndDateRange(
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by campaign and date range: ${error.message}`,
      );
    }
  ...
  async findByErrorCode(
    ...
    if (error) {
      throw new Error(
        `Failed to find delivery logs by error code: ${error.message}`,
      );
    }
  ...
  async getPerformanceStats(dateRange?: {
    ...
    const latencies = (data ?? [])
      .map((log) => log.response_latency_ms)
      .filter((l) => l !== null);
  ...
  private calculateStats(logs: any[]): {
    ...
    const successCount = logs.filter(
      (l) => l.status === DeliveryStatus.Success,
    ).length;
    const failureCount = logs.filter(
      (l) => l.status === DeliveryStatus.Failed,
    ).length;
