import type {
  RateLimiter,
  RateLimitCheck,
} from "@/domain/services/RateLimiter";
import { RateLimit } from "@/domain/services/RateLimiter";
...
export class RateLimiterSupabase implements RateLimiter {
  async check(key: string, rateLimit: RateLimit): Promise<RateLimitCheck> {
    ...
    const windowStart = new Date(
      now.getTime() - rateLimit.windowSeconds * 60 * 1000,
    );
  ...
  async checkLimit(
    ...
    try {
      ...
      if (currentCount < limit) {
        ...
        if (insertError) {
          throw new Error(
            `Failed to record rate limit entry: ${insertError.message}`,
          );
        }
